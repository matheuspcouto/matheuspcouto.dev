...
describe('ProjectsComponent', () => {
  ...
  it('should initialize with projects list using signal', () => {
  ...
  it('should have Igreja Metodista Website project', () => {
    const siteImep = component['projects']().find(p =>
      p.title.toLowerCase().includes('metodista')
    );
    expect(siteImep).toBeDefined();
    expect(siteImep?.url).toContain('igrejametodistapalmas.vercel.app');
  });

  it('should have Hávyla Portfolio project', () => {
    const portfolio = component['projects']().find(p =>
      p.title.toLowerCase().includes('hávyla') || p.title.toLowerCase().includes('havyla')
    );
